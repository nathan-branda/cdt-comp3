...
echo "> Allow TCP/UDP on ports 139, 445, 9000, 12201, 514"

# TCP Inbound
iptables -t mangle -A INPUT -p tcp -m multiport --dports 139,445,9000,12201,514 -m state --state NEW,ESTABLISHED -j ACCEPT
# TCP Outbound
iptables -t mangle -A OUTPUT -p tcp -m multiport --sports 139,445,9000,12201,514 -m state --state ESTABLISHED -j ACCEPT

# UDP Inbound
iptables -t mangle -A INPUT -p udp -m multiport --dports 139,445,9000,12201,514 -m state --state NEW,ESTABLISHED -j ACCEPT
# UDP Outbound
iptables -t mangle -A OUTPUT -p udp -m multiport --sports 139,445,9000,12201,514 -m state --state ESTABLISHED -j ACCEPT
